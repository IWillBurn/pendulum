...
class PhysicsPendulum:
    ...
    def drag_end(self, event):
        if self.is_dragging[1]:
            ...
            main_size = sqrt((event.x - self.main.axis.x)**2 + (event.y - self.main.axis.y)**2) / self.main.params["scale"]
            if size - main_size > 0:
                self.main.size = main_size
                self.main.counterweight_size = size - main_size
    '''
    ...
    def draw(self):
        len_x = cos(self.main.angle + pi / 2) * self.main.size * self.main.params["scale"]
        len_y = sin(self.main.angle + pi / 2) * self.main.size * self.main.params["scale"]

        counterweight_x = cos(self.main.angle + pi / 2) * self.main.counterweight_size * self.main.params["scale"]
        counterweight_y = sin(self.main.angle + pi / 2) * self.main.counterweight_size * self.main.params["scale"]
        ...
        center_len_x = cos(self.main.angle + pi / 2) * part * self.main.params["scale"] + self.main.axis.x
        center_len_y = sin(self.main.angle + pi / 2) * part * self.main.params["scale"] + self.main.axis.y
